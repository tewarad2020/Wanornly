    ...
    filter() {
      ...
      if (list_books) {
        ...
        datafilter.filter((book) => {
          ...
          if (publisher) publisher = publisher.toLowerCase().replaceAll(" ", "");
          ...
          let includePublisher = true;
          for (let i = 0; i < newtext.length; i++) {
            if (!publisher.includes(newtext[i])) {
          ...
          if (name && author && publisher && (includeName || includeAuther || includePublisher)) {
            ...
            bookPublisher.classList.add("bookPublisher");
            ...
            if (book.publisher !== 'no information') box_info.appendChild(bookPublisher);
